...
Runtime mirror: `server/scripts/<videoId>.json`, loaded at server start.
Keep this document and the JSON synchronized when either changes.

Where a script exists:

- **`/api/manual`** serves the script's steps directly (no web search); the cited
  source is the original footage the clip was cut from.
- **`/api/chat`** receives the timeline pinned to the playhead — what is
  happening now, what already happened, what comes next (or `afterClip` past the
  end) — so "what should I do next?" answers are grounded in this exact build,
  not a generic guide.
...
After the clip (for "what next" generations): mount the radiator into the case
top with the fans screwed through the tray, bolt the pump block onto the CPU
using the X bracket over the pre-applied paste, then install the motherboard in
the case, add RAM, GPU, and PSU, and connect front-panel, fan, and pump headers
before first boot.

Derived manual steps (served for "show me the steps" on this clip): see
`steps` in `server/scripts/pov-pc-build-fail.json` — 8 steps, each anchored to
the timeline with an `at` timestamp.
